...
| `test_mcp_read_tools_matrix_live.py` | Same | Listed tools (including `quality_audit` + `sync` `max_files=1`) return without `-32001` and under ~24s on ThinkingSOC scope |
| `test_mcp_tool_payload_quality_live.py` | Same + `/opt/Astloom` pin | Semantic payload checks on **astloom** and **ThinkingSOC**: scope, search hits/scores, sync mode, `quality_audit` not degraded + correct repo pin, architecture/detect/neighbors shape, memory write→retrieve |
