...
| --- | --- | --- |
| `test_unused_candidates_mcp_http_live.py` | MCP HTTP up; `.astloom/mcp-http.secret` | Ingests tiny fixture into project `deadcode-live`, then proves scored unused_candidates (orphan `old_helper_orphan`) + `kpi_hints` / triage. Does **not** prove ThinkingSOC dump budgets — see doc `83`. |
