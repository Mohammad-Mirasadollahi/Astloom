...
from logical_examples_gate.catalog import REQUIRED_DOCS
from logical_examples_gate.checks import run_all_checks
from logical_examples_gate.gate import check_phase_gate, explain_failed_check
...
def test_logical_examples_gate_passes_docs_and_catalog():
...
def test_logical_examples_verification_checks_exit_criteria():
...
def test_waiver_marks_gate_waived_when_forced_failure(monkeypatch):
    ...
    decision = check_phase_gate(waiver_ref="issue:logical-examples-temp")
    assert decision.status == "waived"
    explained = explain_failed_check(decision, "logical-examples-doc-1")
