...
class Settings:
    ...
    def from_environment(cls) -> "Settings":
        database_url = os.environ.get("ASTLOOM_CORE_DATA_DATABASE_URL", "").strip()
        if not database_url:
            raise RuntimeError("ASTLOOM_CORE_DATA_DATABASE_URL is required")
        if not database_url.startswith(("postgresql://", "postgresql+psycopg://")):
            raise RuntimeError("ASTLOOM_CORE_DATA_DATABASE_URL must use PostgreSQL")
...
def build_container(settings: Settings | None = None) -> ServiceContainer:
    ...
    return ServiceContainer(service=CoreData(PostgresStore(resolved.database_url)), settings=resolved)
