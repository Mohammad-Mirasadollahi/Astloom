...
class Settings:
    ...
    def from_environment(cls) -> "Settings":
        database_url = os.environ.get("ASTLOOM_DOCS_SYNC_DATABASE_URL", "").strip()
        if not database_url:
            raise RuntimeError("ASTLOOM_DOCS_SYNC_DATABASE_URL is required")
        if not database_url.startswith(("postgresql://", "postgresql+psycopg://")):
            raise RuntimeError("ASTLOOM_DOCS_SYNC_DATABASE_URL must use PostgreSQL")
...
def build_container(settings: Settings | None = None) -> ServiceContainer:
    ...
    # Pool max auto-probes Postgres inside PostgresStore (or ASTLOOM_PG_POOL_MAX).
    store = PostgresStore(resolved.database_url)
    return ServiceContainer(service=DocsSyncService(store), settings=resolved)


def build_service(settings: Settings | None = None) -> DocsSyncService:
