...
class InMemoryStore:
    ...
    def get_principal(self, principal_id: str, scope: Scope) -> dict[str, Any]:
        item = self._principals.get(principal_id)
    ...
    def list_principals(self, scope: Scope) -> list[dict[str, Any]]:
        items = [p for p in self._principals.values() if self._same(scope, p)]
        return deepcopy(sorted(items, key=lambda p: p["id"]))
