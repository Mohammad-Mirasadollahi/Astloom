...
from .core import ReportingService
...
class Settings:
    ...
    def from_environment(cls) -> "Settings":
        database_url = os.environ.get("ASTLOOM_REPORTING_DATABASE_URL", "").strip()
        if not database_url:
            raise RuntimeError("ASTLOOM_REPORTING_DATABASE_URL is required")
        if not database_url.startswith(("postgresql://", "postgresql+psycopg://")):
            raise RuntimeError("ASTLOOM_REPORTING_DATABASE_URL must use PostgreSQL")
