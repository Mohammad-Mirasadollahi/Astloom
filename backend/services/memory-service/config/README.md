...
Path: `backend/services/memory-service/config`

## Purpose

Service-specific configuration schema and templates. Parent service: `services/memory-service`.
...
Active. `memory-service.example.env` documents the required PostgreSQL connection URL and memory profile settings.

## Runtime Configuration

- `ASTLOOM_MEMORY_SERVICE_DATABASE_URL` is required at service composition time.
