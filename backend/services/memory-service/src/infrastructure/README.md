...
Path: `backend/services/memory-service/src/infrastructure`

## Purpose

Persistence, messaging, provider, and external infrastructure adapters. Parent service: `services/memory-service`.
...
Active PostgreSQL persistence boundary. `memory_service.postgres_store.PostgresStore` implements the service Store port; schema changes remain owned by `migrations/`.
